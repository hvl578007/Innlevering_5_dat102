...
public class KjedetBinaerSokeTre<T extends Comparable<T>> implements BSTreADT<T>, Iterable<T> {
    ...
    private int hogdeRek(BinaerTreNode<T> p) {
        if (p == null) {
            return -1;
        } else {
            int maksVenstreUndertre = hogdeRek(p.getVenstre());
            int maksHogreUnderTre = hogdeRek(p.getHoyre());
            if (maksHogreUnderTre > maksVenstreUndertre) {
                return maksHogreUnderTre + 1;
            } else {
                return maksVenstreUndertre + 1;
            }
        }
    }
}// class
